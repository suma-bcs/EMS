...
def main():
    ...
    persent=StringVar()
    data=[]
    Label(win,text="Search Record",bg="white",fg="teal",font=("Georgia",16)).place(x=108,y=2)
    ...
    for row in read:
        data.append(row[0])
    e0=OptionMenu(win,uno,"Select Emp. Code",*data)
    e0.place(x=140,y=50)
    e0.focus()
    ...
    Button(win,text="Close",bg="white",fg="teal",font=("Georgia",9),command=close).place(x=150,y=200)
    win.bind("<Key>",press)
    win.mainloop()
